...
export const AssessmentPanel = ({ langKey }: Props) => {
  ...
  if (done) {
    ...
    const level = ratio >= 0.8 ? "متقدم" : ratio >= 0.5 ? "متوسط" : "مبتدئ";
    return (
      <div className="rounded-2xl border border-gold/40 bg-surface-2 p-6 text-center space-y-4">
        <Award className="mx-auto h-12 w-12 text-gold" />
        <h3 className="text-2xl font-black text-white">مستواك: <span className="text-gold">{level}</span></h3>
        ...
        <Button onClick={reset} className="gap-2 bg-gold text-black hover:bg-gold-hover">
  ...
  return (
    <div className="rounded-2xl border border-[#1F2937] bg-surface-2 p-5 space-y-4">
      ...
      <div className="h-1.5 w-full overflow-hidden rounded-full bg-background">
        <div className="h-full bg-gold transition-all" style={{ width: `${((idx) / questions.length) * 100}%` }} />
      </div>
